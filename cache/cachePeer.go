...
func (c *CachePeer) Get(ctx context.Context, in *pb.CacheRequest) (out *pb.CacheResponse, err error) {
	key := in.GetKey()
	//验证key合法性
	if key == "" {
	...
	if v, ok := c.Manager.Get(key); ok {
		log.Println("[Cache] hit")
		return &pb.CacheResponse{Value: v.(ByteView).GetByte()}, nil
	} else if c.CallBackFunc != nil {
		view, err := c.storage(key)
		if err != nil {
			return out, err
		}
		return &pb.CacheResponse{Value: view.GetByte()}, nil
	}
...
// func (c *CachePeer) Get(ctx context.Context, in *pb.CacheRequest) (*pb.CacheResponse, error) {
// 	return &pb.CacheResponse{Value: []byte("fuck you!!!" + in.GetKey())}, nil
// }
